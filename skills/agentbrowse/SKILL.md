...
- **Safety guard:** only `http(s)` URLs are allowed, and local/private hosts are blocked by default (`blocked_url`, exit 2) — hostnames are DNS-resolved and checked by IP, so rebinding tricks are caught. To drive a local dev server, set `WEBCLI_ALLOW_LOCAL=1` (this permits loopback/private but **never** the `169.254.x` cloud-metadata range).
